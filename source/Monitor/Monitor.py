...
class Zone:
    def __init__(self, name, mqtt):
        ...
        logging.debug("{:} Loading setpoint monitor zone".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        self.name = name
        ...
        logging.debug("{:} Starting zone thread".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        self.statusChangeThread.start()
        self.subscribeToInputs()
        logging.debug("{:} Setpoint monitor zone loaded".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def statusChangeFunction(self):
        while True:
            ...
            if (self.enabled == None and self.volatileStatus != None) or ((datetime.now() - self.statusChangedTime).total_seconds() > self.statusChangeDelay and self.volatileStatus != self.enabled):
                logging.debug("{:} : Changing status for {:} to '{:}'".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.name, self.volatileStatus))
    ...
    def requestValues(self, name):
        logging.debug("{:} : requestValues({:})".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), name))
    ...
    def publishSetpoint(self, setpoint):
        logging.debug("{:} : publishSetpoint({:}) : {:}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), setpoint, self.name))
        self.mqtt.publish(self.spTopicOut, setpoint)

    def setStatus(self, status):
        logging.debug("{:} : setStatus({:}) : {:}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status, self.name))
    ...
    def setAndPublishSetpoint(self, setpoint):
        logging.debug("{:} : setAndPublishSetpoint({:}) : {:}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), setpoint, self.name))
    ...
    def on_message(self, client, userdata, message):
        ...
        if (message.topic == self.statusTopicIn):
            ...
            logging.debug("{:} : Received status change {:} for {:}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.volatileStatus, self.name))
